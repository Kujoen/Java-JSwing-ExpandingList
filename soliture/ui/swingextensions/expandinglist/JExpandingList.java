 ...
 * 
 *         A JScrollPane which lets you add ExpandableRows to it. !!! IMPORTANT
 *         - Add rows only AFTER you have added the JExpandingList to an parent
 *         Panel and have set it visible.
 *
 */
public class JExpandingList extends JScrollPane {
	...
	private List<ExpandableRow> rowList;

	/**
	 * Constructor
	 * 
	 * @param rowsPerPanel
	 *            How many rows will be displayed in one panel
	 * @param columnsPerRow
	 *            How many columns can one row have
	 */
	...
	private void addRowToList(ExpandableRow rowToAdd) {
		...
		if (!rowToAdd.getComponentList().isEmpty()) {
			for (int i = 0; i < rowToAdd.getComponentList().size(); i++) {
				ExpandingListComponent componentToAdd = (ExpandingListComponent) rowToAdd.getComponentList().get(i);
	...
	// ***********************************************************************************************|
	/**
	 * DO NOT ADD ROWS BEFORE YOU HAVE SET THE LIST TO VISIBLE
	 * 
	 * @param rowToAdd
	 *            Adds rowToAdd and its children to the list
	 */
